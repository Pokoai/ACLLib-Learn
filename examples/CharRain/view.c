...
void printChar()
{
	char tempStr[2];  // 用来装要显示的字符, 数组里还要加一个'\0'字符才算字符串 
//	char *temp=NULL;  // 不知道为何不能用字符串指针 
	beginPaint();
...
//		printf("%d\n", i);
		...
		// top 实际上就是该雨列的头，strInfo[i][1]为尾 
		int top = strInfo[i][0] - strInfo[i][1]; 
		for ( int j = 0; j < strInfo[i][1]; j++, top++ ) {  // 遍历该雨列，遍历次数：strInfo[i][1] 
			// 雨列的头大于0，小于CharNum，屏幕上才有元素，才需要绘制
			// 否则整个雨列火车往下移动，直到火车头达到屏幕顶端，开始绘制，直到绘制完该雨列长度 
			if ( top < 0 || top >= CharNum ) {
				continue;
			}
			// top: 雨列头，charSet[i] + top：将 charSet[i]的字符串指针移到雨列头部
			// 首次是移到头部，随着 top++，以后每次就是依次从头部往后移动一位
			// 1：将指针所指的这 1个字符拷贝给 temp		
			strncpy(tempStr, charSet[i] + top, 1);
			tempStr[1] = '\0';
			
			// 开始是绘制从 top~top+strInfo[i][1] 的字符，
...
//			paintText(i*TEXTSIZE, top*TEXTSIZE, temp.c_str()); // c++
			...
			// top*TEXTSIZE：随着 top++, 同一雨列的不同位置字符 y 轴不同 
